// Country dial codes for the phone-number picker. Flag emoji is derived from the
// ISO-2 code at runtime, so each entry only needs name + iso2 + dial.

export const DEFAULT_DIAL = '+91'; // India
...
// Dial codes sorted longest-first so prefix matching picks the most specific code
// (e.g. +1242 Bahamas before +1 US).
...
// Split a stored number (e.g. "+919876543210") into { dial, national }.
// Falls back to the default dial when no country code is recognised.
export function splitPhone(value) {
    ...
    const plus = '+' + digits;
    const match = BY_DIAL_LENGTH.find((c) => plus.startsWith(c.dial));
    if (match) {
        return { dial: match.dial, national: plus.slice(match.dial.length) };
    }
    // No recognisable code: assume a local number under the default country.
    return { dial: DEFAULT_DIAL, national: digits };
}

// Combine a dial code + national number into the canonical stored form.
// Returns '' when there's no national number, so "required" validation still fires.
